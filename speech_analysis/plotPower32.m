function [X, F] = plotFftPowerSmall(fftLength, equation, windowCustom, overlap, overlapBins, xMinimum, xDivision, xMaximum, yMinimum, yDivision, yMaximum)

	kResults = zeros(overlapBins, fftLength);
	kFFT = kResults;

	for i = 1:overlapBins
		nShift = fftLength * (1-overlap);
		kResults(i,:) = equation((nShift*(i-1)+1):((nShift*(i-1))+fftLength)) .* windowCustom';
